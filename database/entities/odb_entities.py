...
class _DbConstructors:
    ...
    @staticmethod
    def group_construct(name, entity_id):
        entity_attributes = dict(
            _id=entity_id,
            type='group',
            entry_name=name,
...
class DbGroup:
    ...
    def create(self, name):
        collection = self.db[OriginEnvar.show_name]
        created_id, save_data = _DbConstructors().group_construct(name=name,
                                                                  entity_id=DbIds.create_entity_id(name)
                                                                  )
...
if __name__ == '__main__':
    ...
    # xx = DbProject().create(name="MooMoo")
    # DbAsset().create("blue")
    DbGroup().create("environments")
    # print (xx)
