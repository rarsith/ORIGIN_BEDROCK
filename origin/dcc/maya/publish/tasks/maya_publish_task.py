...
class MayaPublishTask(BatchTask):
    def execute(self):
        ...
        asset_breakdown_version = self.options["db_asset_stream_id"]
        asset_breakdown_id = '.'.join([asset_breakdown_version, "breakdown"])
        context_handler.asset_breakdown_id = asset_breakdown_id
        ...
        print("PUBLISHING RESULTS:  ---->>>> ", publish_results)


        print("CREATING ASSET BREAKDOWN VERSION")
        db_publisher.create_asset_breakdown_version(context=context_handler)
        print("DONE CREATING ASSET BREAKDOWN VERSION")


        print("CREATING ASSET STACK VERSION")
        db_publisher.create_stack_version(context=context_handler)
        print("FINISHED CREATING ASSET STACK VERSION")
        pprint.pprint(context_handler.snapshot_session())
