...
elif not CLONED_CONTEXT.task_type == "rigging":
    raise Exception(f"Task missmatch. You are in {CLONED_CONTEXT.task_type} Task!")

else:
    window = origin_publisher.OriginPublisher(context=CLONED_CONTEXT, publish_type="animation_rig", parent=context_window_parent)
    try:
