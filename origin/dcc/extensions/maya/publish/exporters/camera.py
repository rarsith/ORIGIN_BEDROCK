...
class MayaCameraExporter:
    ...
    def __init__(self, options=None, camera_name=None, src_file_path=None, start_frame=None, end_frame=None):
        ...
        if self.options["review_medium"] != "no preview":
            self.start_frame = self.options["review_options"]["frame_range"][0]
            self.end_frame = self.options["review_options"]["frame_range"][1]
        else:
            self.start_frame = self.options["entity_properties"]["frame_in"]
            self.end_frame = self.options["entity_properties"]["frame_out"]

        if self.options is not None:
            self.camera_name = self.options["publish_type"] + ":camera"
    ...
    def open_master_file(self):
        if self.src_file_path is not None:
            ...
            print(f"self.file_path: {self.src_file_path} NOT FOUND! IS NONE")
