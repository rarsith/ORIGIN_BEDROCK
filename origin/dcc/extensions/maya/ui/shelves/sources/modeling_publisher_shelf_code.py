...
def execute():
    ...
    if not cmds.objExists(target_root):
        ...
        window = origin_publisher.OriginPublisher(context=current_context, publish_type="geometry",
...
def modeling_publish():
    context_window_parent = get_maya_main_window()
    check_current_file = MayaSaveSession()

    is_master = check_current_file.check_if_master_opened()

    if is_master:
        QtWidgets.QMessageBox.information(context_window_parent, "WARNING", "Current Scene cannot be re-published, master scene already!. Please Save As.. to make it yours!")
        # return RuntimeError
