...
class EntryPropertiesEditorUI(QtWidgets.QWidget):
    ...
    def create_connections(self):
        ...
        self.properties_viewer.cellChanged.connect(self.save_warning)

    def save_warning(self):
        element = "save_changes"
        self.changes_to_database.append(element)
        self.check_changes()
    ...
    def get_entry_properties(self):
        if self.entity_received is None:
            return {}

        curr_asset_type = self.entity_received.type
        if curr_asset_type != "group":
            definition_data = self.entity_received.definition
